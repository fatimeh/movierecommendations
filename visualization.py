...
def runner_questions() -> Dict[str, Any]:
    ...
    create_genres_listbox(window, answers_so_far)
    create_duration_listbox(window, answers_so_far)
    create_year_listbox(window, answers_so_far)
    create_language_listbox(window, answers_so_far)
...
def create_genres_listbox(window: tk.Tk, user_answer: dict) -> None:
...
def create_duration_listbox(window: tk.Tk, user_answer: dict) -> None:
    ...
    def submit() -> None:
        ...
        user_answer['duration'] = DURATIONS[duration_listbox.curselection()[0]]
...
def create_language_listbox(window: tk.Tk, user_answer: dict) -> None:
...
def runner_rankings() -> list:
    ...
    ranking_listbox.insert(1, 'genre')
    ranking_listbox.insert(2, 'release_year')
    ranking_listbox.insert(3, 'language')
    ranking_listbox.insert(4, 'duration')

    def submit() -> None:
        """Collect user selection and delete the option from listbox."""
        rank.append(RANKINGS[ranking_listbox.curselection()[0]])
        RANKINGS.remove(RANKINGS[ranking_listbox.curselection()[0]])
